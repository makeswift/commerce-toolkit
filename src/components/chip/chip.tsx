...
export function Chip({ name, value, children, removeLabel = 'Remove', onClick }: ChipProps) {
  return (
    <span className="flex h-9 items-center gap-1.5 rounded-lg bg-[var(--chip-background,hsl(var(--contrast-100)))] py-2 pe-2 ps-3 text-sm font-[var(--chip-font-family,var(--font-family-body))] font-semibold leading-5 text-[var(--chip-text,hsl(var(--foreground)))]">
      {children}
      <button
