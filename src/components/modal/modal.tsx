...
export function Modal({ isOpen, setOpen, title, trigger, children }: ModalProps) {
  return (
    <Dialog.Root onOpenChange={setOpen} open={isOpen}>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-30 flex items-center justify-center bg-[var(--modal-overlay-background,color-mix(in_oklab,hsl(var(--foreground))_50%,transparent))] @container">
          <Dialog.Content
            className={clsx(
